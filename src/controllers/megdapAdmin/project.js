...
exports.getUserProjects = async (req, res) => {
  try {
    ...
    const projectQuery = projectRef
      .where("userId", "==", userId)
      .orderBy("createdAt", "desc")
      .where("status", "==", "In Progress");

    const projectSnapshot = await projectQuery.get();
    if (projectSnapshot.empty) return res.status(200).json({ projects: [] });
...
exports.getPaymentPendingProjects = async (req, res) => {
  try {
    ...
    if (projectsSnapshot.empty) return res.status(200).json({ projects: [] });
...
exports.updatePaymentPendingProjects = async (req, res) => {
  try {
    ...
    if (!projects) return res.status(400).json({ message: "Invalid Request" });
