...
exports.addNewTranslator = async (req, res) => {
    try {
        ...
        const sourceLanguages = languages.map((language) => language.sourceLanguage)
...
exports.getTranslatorsSourceLanguages = async(req,res) =>{
    try {
        ...
        const sourceLanguages = [...new Set(translatorSnapshot.docs.flatMap((doc) => doc.data().sourceLanguages))]
...
exports.getTargetLanguages = async(req,res) =>{
    try {
        ...
        const translatorPairSnapshot = await translatorPairRef.where('sourceLanguage','==',sourceLanguage).get();

        if(translatorPairSnapshot.empty)
            return res.status(200).json({targetLanguages:[]})

        const targetLanguages = [...new Set(translatorPairSnapshot.docs.map((doc) => doc.data().targetLanguage))]
...
exports.getTranslators = async(req,res) =>{
    try {
        ...
        const translatorPairRef = db.collection('translatorPairs');
        const translatorPairSnapshot = await translatorPairRef.where('sourceLanguage','==',sourceLanguage).where('targetLanguage','==',targetLanguage).get();
